...
def clear(df,col,val):
    ...
    """
    value_counts=df[:][df[col]==val].count()
    return value_counts
...
def convert(df,celsius):
    ...
    """
    converted_temp=df[celsius].apply(lambda x: round(9*x/5+30,2))
...
cloudy_weather_distribution=clear(weather,'Weather','Cloudy')
print('cloudy weather distribution: ',cloudy_weather_distribution.count(),' out of :',weather['Weather'].count())
...
print("Number of Instances wind speed was above 35 and visibility was 25: ",wind_speed_35_vis_25.count())
...
time_temp_mean=agg_values_ina_month(weather,'Date/Time','Temp (C)','mean')
print("Mean Temperature recorded by month: ")
print(time_temp_mean)
...
agg1={'Temp (C)':'mean','Dew Point Temp (C)':'mean','Rel Hum (%)':'mean','Wind Spd (km/h)':    'mean','Visibility (km)':'mean', 'Stn Press (kPa)':'mean'}
mean_weather=group_values(weather,'Weather',agg1)
print('Data group by Weather and its mean value: ')
print(mean_weather.head(5))
...
cel_to_farh=pd.DataFrame(convert(weather,'Temp (C)'))
print("Celcius temp covet to temp: ")
print(cel_to_farh.head(5))
